...
//    VALUES ('".$_SESSION['logged_id']."', '".$total_price."', '".$total_price."', '2', '".$recipe_name."',
//     '".$grinding_type."', '".$cust_own_desc."', '".date("Y-m-d H:i:s")."', '".date("Y-m-d H:i:s")."')");

  $updateRecipe  = mysqli_query($con, "UPDATE  reacipies  set customer_id='".$_SESSION['logged_id']."', subtotal='".$total_price."', total_price='".$total_price."', order_form=2, recipe_name='".$recipe_name."', 
                            grinding_type='".$grinding_type."', cust_own_desc='".$cust_own_desc."', created='".date("Y-m-d H:i:s")."', modified='".date("Y-m-d H:i:s")."' WHERE id='".$recipeId."' ");

  if($updateRecipe) {
      $removeRecipeItems  = mysqli_query($con, "DELETE from reacipie_items where reacipie_id='".$recipeId."'");
      if(mysqli_error($con)) {
                ...
                <div class="container">
                 ...
                 <form method="post" autocomplete="off">  
                   <?php  
                      if(isset($_GET['id'])) {
                          $recipeId   = $_GET['id'];
                          $recipeQuery  = mysqli_query($con, "SELECT * from reacipies where id='".$recipeId."' ");
                        ...
                        <div class="col-xl-8">
                            <div class="cart-wrapper">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                           <input type="text" value="<?= $recipeDetails['recipe_name'] ?>" class="form-control" name="recipe_name" id="recipe_name" placeholder="Name your seasoning" required> 
                                ...
                                <div class="table-responsive">
                                    <table class="table mb-0">
                                        ...
                                        <tbody id="table-body">
                                            <?php   $recipeItemsQuery = mysqli_query($con, "SELECT * from reacipie_items where reacipie_id='".$recipeId."' "); 
                                                
                                                $count = 1;
                                                while ($recipeItems  = mysqli_fetch_assoc($recipeItemsQuery)) {
                                                    # code...
                                                  ?>
                                            <tr class="single-row">
                                                ...
                                                <select class="form-control spices_list" name="spices_ids[]" id="spices_ids1" onchange="spices_image(1,this.value)" required>
                                                    ...
                                                    <?php if($spices_list_row['id']  == $recipeItems['product_id'] ) { ?>
                                                        <option selected value="<?php echo $spices_list_row['id']; ?>"><?php echo $spices_list_row['spices_name']; ?></option>
                                                ...
                                                <input type="hidden" class="spices_inv_total" name="spices_inv_total[]" id="spices_inv_total<?=$count?>" value="<?= $recipeItems['product_price'] ?>">
                                                <td class="product-quantity">
                                                    <div class="shop-perched-info">
                                                        <div class="sd-cart-wrap">
                                                          <div class="cart-plus-minus">      
                                                                <input type="text" value="<?= $recipeItems['quantity'] ?>" class="qty" placeholder="0" name="prd_qty[]" id="prd_qty1" onchange="update_quantity(1)"  required>
                                                           </div>         
                                                        </div>
                                                        <p>Grams</p>
                                                    </div>
                                                ...
                                                <td class="product-subtotal">
                                                    <input type="radio" name="raw_roasted<?=$count?>" <?php if($recipeItems['spices_type'] == 'Raw') { ?> checked <?php } ?> id="raw_roasted<?=$count?>" value="Raw" required>&nbsp;Raw&nbsp;&nbsp;
                                                    <input type="radio" name="raw_roasted<?=$count?>" <?php if($recipeItems['spices_type'] == 'Roasted') { ?> checked <?php } ?> id="raw_roasted<?=$count?>" value="Roasted">&nbsp;Roasted
                                                </td>
                                        ...
                                        <?php $count++; } ?>
                                        <input type="hidden" name="" id="total-rows" value="<?= $count ?>">
                                        <tr>
                                               <td colspan="2">
                            ...
                            <div class="row" style="margin-top:15px;">
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        <input type="radio" name="grinding_type"  <?php if($recipeDetails['grinding_type']  == 'Powder') {  ?> checked <?php } ?> id="grinding_type" value="Powder" required>&nbsp;Powder&nbsp;
                                        <input type="radio" name="grinding_type" id="grinding_type" <?php if($recipeDetails['grinding_type']  == 'Coarse') {  ?> checked <?php } ?> value="Coarse" >&nbsp;Coarse&nbsp;
                                        <input type="radio" name="grinding_type" id="grinding_type" <?php if($recipeDetails['grinding_type']  == 'Semi Coarse') {  ?> checked <?php } ?> value="Semi Coarse" >&nbsp;Semi Coarse&nbsp;
                                        <input type="radio" name="grinding_type" id="grinding_type"  <?php if($recipeDetails['grinding_type']  == 'Flakes') {  ?> checked <?php } ?>value="Flakes" >&nbsp;Flakes&nbsp;
                                    </div>
                                </div>
                                ...
                                <div class="shop-cart-widget">
                                    <div class="form">
                                        <ul>
                                            ...
                                            <li class="sub-total"><span>Subtotal</span> <span class="amount">£ <?= $recipeDetails['subtotal'] ?> </span></li>
                                            <li class="cart-total-amount"><span>Total Price</span> <span class="amount">£ <?= $recipeDetails['total_price'] ?> </span></li>
                                        </ul><br>
           ...
           function increment_quantity(cart_id) {
            ...
            function decrement_quantity(cart_id) {
             ...
             function update_quantity(cart_id) {
                ...
                let t = document.getElementById("total-rows").value;
